...
per op in nanoseconds, and writes a report to `build/reports/benchmarks/`.
Configuration lives in `build.gradle.kts` under `benchmark { configurations { ... } }`
(warmups, iterations, iteration time). JMH guards against dead-code
elimination, constant folding, and on-stack replacement skew.

This is the number to quote in PRs and release notes.
